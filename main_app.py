...
# A importação do gravador de áudio foi removida.

import database_utils
import openai_utils

# A função 'processar_gasto' foi removida, pois a lógica voltará para dentro do chat_input.
...
with tab1:
    ...
    if 'pending_expense' in st.session_state and st.session_state.pending_expense:
        ...
        # A mensagem de info foi simplificada.
...
if 'pending_expense' not in st.session_state or not st.session_state.pending_expense:
    if prompt := st.chat_input("Digite o gasto aqui..."):
        # A lógica voltou para dentro do chat_input
        if "messages" not in st.session_state:
            st.session_state.messages = []
        st.session_state.messages.append({"role": "user", "content": prompt})
        
        with st.spinner("Analisando despesa..."):
            analysis = openai_utils.analyze_expense_text(prompt, CATEGORIES)
